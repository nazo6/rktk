...
pub fn aml() {
    ...
    assert_eq!(report.highest_layer, 0, "Mouse moved a bit, aml not activated");

    let report = state.update(InputEvent::Mouse((1, 1)), time(0));
    assert_eq!(report.highest_layer, 0, "Mouse moved a bit again, aml not activated");

    let _ = state.update(InputEvent::Mouse((1, 1)), time(0));
    let report = update!(state, time(0));
    assert_eq!(report.highest_layer, 1, "Mouse moved once again, aml activated");
}
...
pub fn aml_reset() {
    ...
    let report = update!(state, time(0));
    assert_eq!(report.highest_layer, 0, "Aml reset key pressed. aml deactivated");
}
