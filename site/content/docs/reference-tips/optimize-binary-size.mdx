...
- `panic-immediate-abort`
  ...
  With this option enabled, the program will immediately abort on panic without unwinding or running destructors. This can further reduce binary size compared to the standard `abort` strategy.

  Note that this option is only available in nightly rust. Also you need to specify `panic-immediate-abort = true` in `[unstable]` section to use this feature.
